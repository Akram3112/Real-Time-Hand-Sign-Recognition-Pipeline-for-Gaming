...
def load_split(split_name):
    path = os.path.join(SPLIT_PATH, f"{split_name}.csv")
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Missing split file: {path}. Run split_dataset.py first."
        )
    ...
    if "label" not in df.columns:
        raise RuntimeError(f"Missing 'label' column in split file: {path}")

    feat_cols = [c for c in df.columns if c not in ("person_id", "label")]
    X_split = df[feat_cols].to_numpy(dtype=float)

    # Skip bad rows defensively
    valid_rows = np.isfinite(X_split).all(axis=1)
    X_split = X_split[valid_rows]
    labels = df.loc[valid_rows, "label"].to_numpy()

    if len(X_split) == 0:
        raise RuntimeError(f"No frame-level samples found in split: {split_name}")

    y_split = le.transform(labels)
...
missing = set(range(len(le.classes_))) - test_classes
if missing:
    print(f"\nWarning: missing from test set: {[le.classes_[i] for i in missing]}")
else:
...
checkpoint_path = os.path.join(RESULTS_PATH, "mudra_mlp_frame_level.pt")
torch.save(
    {
...
cm_path = os.path.join(RESULTS_PATH, "confusion_matrix_frame_level.png")
...
plt.tight_layout()
curves_path = os.path.join(RESULTS_PATH, "training_curves_frame_level.png")
